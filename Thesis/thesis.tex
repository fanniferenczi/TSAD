...
Best F1 and best AU-PR per dataset in \textbf{bold}.}
\label{tab:accuracy}
\begin{tabular}{ll rrrrr}
\toprule
\textbf{Dataset} & \textbf{Metric} & \textbf{TimesNet} & \textbf{ModTCN} & \textbf{TranAD} & \textbf{AnomTr.} & \textbf{GTA} \\
\midrule
\multirow{4}{*}{SMD}
    & Precision & 87.57 & 86.84 & 90.72 & 89.41 & 86.17 \\
    & Recall    & 80.35 & 80.01 & 99.73 & 96.20 & 95.67 \\
    & F1        & 83.80 & 83.28 & \textbf{95.01} & 92.68 & 90.67 \\
    & AU-PR     & 14.90 & 14.20 & \textbf{50.19} &  4.45 & 15.82 \\
\midrule
\multirow{4}{*}{MSL}
    & Precision & 87.04 & 89.39 & 90.38 & 98.89 & 91.69 \\
    & Recall    & 71.40 & 74.80 & 99.99 & 98.00 & 84.24 \\
    & F1        & 78.45 & 81.45 & \textbf{94.95} & 94.91 & 87.81 \\
    & AU-PR     & 14.85 & 14.80 & \textbf{16.59} & 10.42 & 11.62 \\
\midrule
\multirow{4}{*}{SMAP}
    & Precision & 89.49 & 90.29 & 80.43 & 93.67 & 73.66 \\
    & Recall    & 55.44 & 53.85 & 99.99 & 99.32 & 78.48 \\
    & F1        & 68.47 & 67.46 & 89.15 & \textbf{96.41} & 76.00 \\
    & AU-PR     & 11.86 & 11.36 & 10.16 & \textbf{12.90} & 12.33 \\
\midrule
\multirow{4}{*}{SWaT}
    & Precision & 90.29 & 95.85 & 99.77 & 98.88 & 90.27 \\
    & Recall    & 95.41 & 86.97 & 68.79 & 87.00 & 88.36 \\
    & F1        & \textbf{92.78} & 91.19 & 81.43 & 92.56 & 89.31 \\
    & AU-PR     & 10.81 &  9.60 & \textbf{73.66} & 11.10 &  8.62 \\
\midrule
\multirow{4}{*}{PSM}
    & Precision & 98.37 & 98.74 & 89.44 & 97.16 & 99.27 \\
    & Recall    & 93.46 & 94.32 & 85.63 & 97.47 & 93.71 \\
    & F1        & 95.85 & 96.48 & 87.50 & \textbf{97.31} & 96.41 \\
    & AU-PR     & 37.80 & 38.29 & \textbf{46.01} & 28.84 & 34.97 \\
\midrule
\multirow{2}{*}{Average}
    & F1        & 83.87 & 83.97 & 89.61 & \textbf{94.77} & 88.04 \\
    & AU-PR     & 18.04 & 17.65 & \textbf{39.32} & 13.54 & 16.67 \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:f1_boxplot} and Figure~\ref{fig:aupr_boxplot} complement the per-dataset results in 
...
\begin{figure}[ht]
    ...
    point-adjust evaluation protocol. Each marker represents a single dataset result.}
    \label{fig:f1_boxplot}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{aupr_boxplot_by_model.png}
    \caption{AU-PR distribution per model across the five benchmark datasets. 
    Each marker represents a single dataset result.}
    \label{fig:aupr_boxplot}
\end{figure}

The F1 boxplot reveals that Anomaly Transformer achieves not only the highest median F1 but also 
the narrowest interquartile range among all models, indicating consistent strong performance across 
datasets. Its scores are tightly clustered between 92\% and 97\%. TranAD shows a wider spread, 
reflecting its strong performance on SMD and MSL contrasted with a notably lower F1 on SWaT (81.43\%). 
TimesNet and ModernTCN exhibit very similar distributions, both characterised by a wide interquartile range 
driven by their weak performance on SMAP (68.47\% and 67.46\% respectively). 
GTA presents a relatively compact distribution with a high median, 
though it similarly struggles on SMAP, which appears as a pronounced outlier at 76.00\%, 
while its PSM result (96.41\%) appears as a positive outlier at the upper end. It is worth 
noting that the individual dataset markers for SMAP and PSM are not connected by the standard boxplot whiskers. 
This occurs because with only five data points per model, any observation that falls sufficiently 
far from the interquartile range is rendered as a standalone outlier marker rather than being 
included within the whisker extent, following the conventional 1.5$\times$IQR rule.

The AU-PR boxplot tells a strikingly different story. Four of the five models --- TimesNet, 
ModernTCN, Anomaly Transformer, and GTA --- display very compressed distributions clustered 
between roughly 8\% and 16\%, with PSM as a consistent positive outlier for all of them. This 
...
exception: its interquartile range spans approximately 16\% to 50\%, and its SWaT AU-PR of 
73.66\% appears as a high outlier, driving both its superior average AU-PR and the large 
variance in its distribution. Notably, Anomaly Transformer's SMD AU-PR (4.45\%) appears as a 
strong negative outlier, highlighting that its high F1 on that dataset is highly sensitive to 
threshold choice. Taken together, the two plots suggest that high point-adjusted F1 and high 
AU-PR are not interchangeable: a model can rank first on F1 while performing near the bottom 
on AU-PR, reinforcing the importance of reporting both metrics \citep{darban2024survey}.

\subsubsection{What's behind TranAD's high AU-PR on SWaT}
Figure~\ref{fig:pr_tranad_swat} shows the precision-recall curve for TranAD on the SWaT dataset and the no-skill baseline.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{pr_curve_TranAD_SWaT.png}
    \caption{Precision-recall curve for TranAD on SWaT. The dashed line indicates
    the no-skill baseline equal to the dataset's anomaly rate (12.9\%).}
    \label{fig:pr_tranad_swat}
\end{figure}
...
SWaT \citep{trad2022}. The steep vertical drop around recall 0.6--0.65 marks the point at 
...
\subsubsection{TODO: What type of anomalies were detected by each model? why?}

\subsection{TODO: Memory Usage}

\subsection{TODO: Inference Time}
