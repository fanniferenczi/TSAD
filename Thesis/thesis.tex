...
Figure~\ref{fig:f1_stripplot} and Figure~\ref{fig:aupr_stripplot} complement the per-dataset results in 
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{f1_stripplot_by_model.png}
    ...
    \label{fig:f1_stripplot}
\end{figure}

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{aupr_stripplot_by_model.png}
    \caption{AU-PR distribution per model across the five benchmark datasets. 
    Each marker represents a single dataset result.}
    \label{fig:aupr_stripplot}
\end{figure}

The F1 strip plot reveals that Anomaly Transformer achieves the highest scores and the most 
consistent performance across datasets, with its points tightly clustered between 92\% and 97\%. 
...
The AU-PR strip plot tells a strikingly different story. Four of the five models --- TimesNet, 
ModernTCN, Anomaly Transformer, and GTA --- have their points tightly clustered between 
...
exception: its points are widely spread from approximately 10\% to 74\%, with its SWaT AU-PR 
