...
\today \\[1ex] Advisors: Qi Zhang, Gin\'es Carreto Pic\'on, Ira Assent\\[15ex]}\\[\fill]}
...
resource-constrained IoT edge devices remains underexplored. This thesis
...
F1 score, while TranAD leads on AU-PR. A high AU-PR implies that the model is
robust to threshold choice, performing well across a wide range of decision
boundaries rather than at a single operating point. Notably, the two
...
competitive latency on both environments despite executing entirely on the CPU. 
ModernTCN achieves the best latency and
throughput on both server-grade GPU environment and
resource-constrained edge device and presents the most balanced overall profile.
...
maintenance \citep{choi2021deeplearning}, security monitoring \citep{jeffrey2023review},
and system optimisation \citep{sgueglia2022iot} across a wide range of
...
This thesis addresses this gap by asking: how do models that achieve high accuracy
on benchmark datasets perform when evaluated against metrics that matter for IoT
edge deployment --- memory footprint, arithmetic cost, and inference speed on
constrained hardware? The goal is not only to rank models on accuracy, but to
understand what architectural properties make a model suitable or unsuitable for
real-world edge deployment.

This thesis aims to answer this question by benchmarking five deep learning models for
multivariate time series anomaly detection with an explicit focus on their
suitability for IoT edge deployment. The five selected models --- TimesNet
...
represent a diverse range of architectures, spanning
...
Temporal anomalies are deviations that are localised in the time dimension,
meaning they occur at a specific point or subsequence along the time axis rather
than across sensor dimensions.
...
\begin{itemize}
    ...
    A deviation from a local neighbourhood --- a value that is normal in one context but anomalous in another 
    (e.g., a high number of restaurant customers is anomalous on a regular weekday but perfectly expected on a weekend).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Deep Learning Models for Time Series Anomaly Detection}
...
\begin{itemize}
    ...
    detection in univariate time series, and LSTM-NDT \citep{hundman2018smap}, which 
    combines LSTM with Nonparametric Dynamic Thresholding (NDT) \citep{hundman2018smap} for spacecraft anomaly detection. 
    ...
    OmniAnomaly \citep{su2019smd} uses a VAE with stochastic RNNs for robust 
    ...
    errors and distinguish anomalies more reliably. RANSynCoders \citep{abdulaal2021psm} 
...
Common thresholding methods applied to anomaly scores include NDT \citep{hundman2018smap} and POT \citep{siffer2017pot}.
...
the Precision-Recall Curve (AU-PR). These metrics are defined in terms
...
constitute only a small fraction of all observations \cite{darban2024survey}. AU-PR focuses 
directly on the model's ability to retrieve the rare anomalous
...
possibility of deployment on cheaper or more constrained hardware \citep{camus2019review}.
...
cross-model comparison rather than absolute operation count.
...
rather than by the number of parameters. For TimesNet, ModernTCN, Anomaly Transformer,
and GTA, the overhead is computed from GPU VRAM measurements. For TranAD, it is
computed from CPU RAM.
...
\section{Key Contributions and Thesis Structure}
The main contributions of this thesis are as follows. Five deep learning models are benchmarked 
not only on detection accuracy but on the full set of efficiency constraints that determine whether 
a model is viable for IoT edge deployment, revealing a different picture than the 
accuracy leaderboard alone would suggest. Detection accuracy is assessed using both point-adjusted 
F1 score and AU-PR score computed from raw anomaly scores without point adjustment, allowing a direct 
comparison between the threshold-sensitive PA protocol and a threshold-independent measure 
of score separability --- revealing cases where models that appear strong under F1 perform poorly 
when evaluated without the benefit of segment-level credit. Static and runtime memory usage are 
profiled on both server and edge hardware, enabling identification of which models are feasible 
under realistic deployment constraints and which are not. MAC counts are measured to characterise 
the computational cost of each model independently of hardware, identifying which architectures 
are inherently expensive to run. Finally, latency and throughput are measured on both server and 
edge device environments, 
providing direct guidance on which models can sustain real-time inference on constrained hardware.

Having established the background on time series anomaly detection, deep learning architectures, 
and evaluation metrics, the remainder of this thesis is organised as follows. 
Chapter~\ref{ch:selected_models} describes the five models evaluated in this thesis, 
covering their architectural design and anomaly detection approach. Chapter~\ref{ch:experiments} 
presents the experimental setup, datasets, and results across both accuracy and efficiency dimensions. 
Chapter~\ref{ch:conclusion} concludes the thesis with a summary of findings, a deployment recommendation, 
and directions for future work.

\chapter{Selected Models}
\label{ch:selected_models}
...
architectural diversity, and relevance to IoT edge deployment. All five models operate under 
an unsupervised learning scheme, trained 
exclusively on normal data without any anomaly labels. Table~\ref{tab:model_summary} 
summarises the key characteristics of each model.
...
\begin{table}[ht]
\centering
\caption{Summary of selected model characteristics.}
\label{tab:model_summary}
\begin{tabular}{lll}
\toprule
\textbf{Model} & \textbf{Architecture} & \textbf{Approach} \\
\midrule
TimesNet \citep{timesnet2023}              & CNN               & Reconstruction \\
ModernTCN \citep{moderntcn2024}            & CNN               & Reconstruction \\
TranAD \citep{tranad2022}                  & Transformer       & Reconstruction \\
Anomaly Tran. \citep{anomalytransformer2022} & Transformer     & Reconstruction \\
GTA \citep{gta2022}                        & Transformer + GNN & Forecasting    \\
\bottomrule
\end{tabular}
\end{table}
...
\section{Summary}
This chapter described the five deep learning models evaluated in this thesis. 
TimesNet \citep{timesnet2023} and ModernTCN \citep{moderntcn2024} represent the 
CNN-based family, both employing reconstruction-based 
anomaly detection but differing fundamentally in how they process temporal information: 
TimesNet through FFT-guided 2D transformation and ModernTCN through large-kernel depthwise 
convolution with an enlarged effective receptive field. TranAD \citep{tranad2022} and 
Anomaly Transformer \citep{anomalytransformer2022}
represent the Transformer-based family, both designed exclusively for anomaly detection 
but with contrasting detection philosophies: TranAD amplifies reconstruction errors through 
adversarial self-conditioning, while Anomaly Transformer measures the discrepancy between 
a time point's learned and prior-expected attention distributions. GTA \citep{gta2022} combines a 
Transformer-based encoder-decoder with a hierarchical graph convolution context encoding, 
distinguishing itself from the other models as the only forecasting-based model and 
the only one to explicitly learn inter-sensor dependencies as a graph topology, 
motivated by the cascading failure dynamics characteristic of IoT systems. Together, 
the five models span convolutional, transformer-based, and graph-based architectures 
and two detection paradigms, providing a diverse basis for the accuracy and efficiency 
evaluation presented in Chapter~\ref{ch:experiments}.
...
8\,GB of VRAM; all experiments used a single GPU. TranAD, whose codebase hardcodes CPU execution, 
ran exclusively on the server CPUs.

The edge device is an NVIDIA Jetson AGX Orin running Ubuntu 22.04.5 LTS, 
equipped with a 6-core ARMv8 processor (1 thread per core, 1.73\,GHz maximum clock) 
and 7.4\,GB of unified memory shared between the CPU and the integrated Jetson GPU.
...
The AU-PR bar chart tells a different story. Four of the five models --- TimesNet, 
...
fully detected \citep{darban2024survey}, which substantially changes the effective recall and explains 
...
27.8\% baseline across the full recall range on PSM. TranAD
...
At inference time, each model produces a continuous anomaly score for every timestep, 
and a fixed threshold $\delta$ is used to convert these scores into binary anomaly 
labels: any timestep whose score exceeds $\delta$ is flagged as anomalous. The threshold 
$\delta$ is determined from the validation set by selecting the top $r = 1\%$ of 
validation anomaly scores as anomalous \citep{anomalytransformer2022}. The same 
$\delta$ is then applied unchanged at test time. However, the anomaly scores themselves 
are computed differently across the three models.
...
\multirow{3}{*}{SMD}
    ...
    & W\textsubscript{ex} (MB)  &  28.16 & \underline{2.61} & \textbf{1.02} & 18.42 &  6.08 \\
    & W\textsubscript{alloc}$^\dagger$ (MB) &  30.44 & \underline{2.65} & \textbf{1.03} & 28.77 & 15.88 \\
\midrule
\multirow{3}{*}{MSL}
    & Params (\#)                 & 7{,}045{,}559 & 1{,}064{,}264 & \textbf{261{,}243} & 4{,}863{,}055 & \underline{844{,}523} \\
    & W\textsubscript{ex} (MB)  &  28.28 & \underline{4.26} & \textbf{2.09} & 18.55 &  6.44 \\
    & W\textsubscript{alloc}$^\dagger$ (MB) &  30.46 & \underline{4.32} & \textbf{2.10} & 28.90 & 16.26 \\
\midrule
\multirow{3}{*}{SMAP}
    & Params (\#)             & 1{,}761{,}753 & \underline{143{,}016 } & \textbf{57{,}273} & 4{,}801{,}585 & 769{,}253 \\
    & W\textsubscript{ex} (MB)  & 7.05 &  \underline{0.57} & \textbf{0.46} & 18.32 &  5.87 \\
    & W\textsubscript{alloc}$^\dagger$ (MB) &  7.70 &  \underline{0.59} & \textbf{0.46} & 28.67 & 15.66 \\
\midrule
\multirow{3}{*}{SWaT}
    & Params (\#)             & 7{,}044{,}531 & 960{,}840 & \textbf{225{,}519} & 4{,}854{,}859 & \underline{832{,}407} \\
    & W\textsubscript{ex} (MB)  &  28.18 & \underline{3.84} & \textbf{1.80} & 18.52 &  6.35 \\
    & W\textsubscript{alloc}$^\dagger$ (MB) &  30.45 & \underline{3.90} & \textbf{1.81} & 28.87 & 16.16 \\
\midrule
\multirow{3}{*}{PSM}
    & Params (\#)             & 1{,}761{,}753 & \underline{143{,}016} & \textbf{57{,}273} & 4{,}801{,}585 & 769{,}253 \\
    & W\textsubscript{ex} (MB)  &  7.05 &  \underline{0.57} & \textbf{0.46} & 18.32 &  5.87 \\
...
\multirow{3}{*}{Average}
    ...
    & W\textsubscript{ex} (MB)  &  19.74 & \underline{2.37} & \textbf{1.17} & 18.43 &  6.12 \\
    & W\textsubscript{alloc}$^\dagger$ (MB) &  21.35 & \underline{2.41} & \textbf{1.17} & 28.78 & 15.92 \\
\bottomrule
...
with its values spanning from 7.70\,MB on SMAP and PSM to 30.46\,MB on SMD, MSL, and SWaT. 
...
This variability in TimesNet is not incidental but a direct consequence of 
...
all sensor pairs. This matrix scales with the number of input features,
meaning that datasets with more sensors (SMD, MSL, SWat) result in a larger 
learnable graph structure than lower-dimensional
datasets such as SMAP and PSM. 
...
W\textsubscript{alloc} consistently exceeds W\textsubscript{ex}.

From a deployment perspective, GTA's average W\textsubscript{alloc} of 15.92\,MB
places it behind both TranAD (1.17\,MB) and ModernTCN (2.37\,MB). Unlike TimesNet and
...
\begin{tabular}{ll rrrrr}
\toprule
\textbf{Dataset} & \thead{\textbf{Metric}\\(MB) $\downarrow$}
    & \thead{\textbf{TimesNet}\\\citep{timesnet2023}}
    & \thead{\textbf{ModTCN}\\\citep{moderntcn2024}}
    & \thead{\textbf{TranAD}$^\dagger$\\\citep{tranad2022}}
    & \thead{\textbf{AnomTr.}\\\citep{anomalytransformer2022}}
...
Anomaly Transformer's W\textsubscript{alloc} of  28.78\,MB, while the
...
(batch size = 64). Memory values in MB; parameter count in absolute numbers.}
\label{tab:memory_runtime_edge_anomtr}
\begin{tabular}{l rrrrrr}
\toprule
\textbf{Dataset}
    & \thead{\textbf{Params}\\(\#)}
    & \thead{\textbf{W\textsubscript{ex}}\\(MB)}
    & \thead{\textbf{W\textsubscript{alloc}}\\(MB)}
    & \thead{\textbf{Peak train}\\(MB)}
    & \thead{\textbf{Peak infer}\\(MB)}
    & \thead{\textbf{OH}\\(MB)} \\
\midrule
SMD  & 4{,}828{,}222 & 18.42 & 28.30 & 1552.57 & 1484.07 & 1524.27 \\
MSL  & 4{,}863{,}055 & 18.55 & 28.43 & 2120.92 & 1548.87 & 2092.49 \\
SMAP & 4{,}801{,}585 & 18.32 & 28.20 & 2118.75 & 1543.49 & 2090.55 \\
SWaT & 4{,}854{,}859 & 18.52 & 28.40 & 2122.15 & 1548.75 & 2093.74 \\
PSM  & 4{,}801{,}585 & 18.32 & 28.20 & 2118.70 & 1543.49 & 2090.50 \\
\midrule
Avg  & 4{,}829{,}861 & 18.43 & 28.31 & 2006.62 & 1533.73 & 1978.31 \\
\bottomrule
\end{tabular}
\end{table}
...
for instance element-wise arithmetic (\texttt{aten::mul}). All MAC counts
should consequently be interpreted as lower bounds on the true per-forward-pass
cost, suitable for relative cross-model comparison rather than absolute operation count.
...
31{,}357 kMACs with little variation across datasets. This places it third in the 
ranking, behind TranAD and ModernTCN but well ahead of 
Anomaly Transformer and TimesNet.
...
multiple inference runs on the test set to reduce timing variance.
...
low MAC count and GPU-accelerated execution.
...
Figures~\ref{fig:latency_bar} and~\ref{fig:throughput_bar} visualise the cross-environment
shift in latency and throughput, with the latency increase and throughput reduction factors 
annotated directly
on each bar. On the server, ModernTCN and TranAD are the two fastest models, separated by
under 2\,ms. On the edge device, ModernTCN retains its position as the fastest model at
58.6\,ms with a 6.0$\times$ latency increase, while TranAD falls back to 107.8\,ms with a
9.9$\times$ latency increase, indicating that the edge device's CPU is substantially less
capable than the server CPU, amplifying the cost of CPU-only execution in a constrained
environment.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{latency.png}
    \caption{Average latency (ms) for all five models on the server and edge device,
    averaged across the five benchmark datasets. The latency increase factor (edge / server)
    is annotated on each edge bar. Lower is better ($\leftarrow$).}
...
\begin{figure}[H]
    ...
    \caption{Average throughput (input windows/s) for all five models on the server
    and edge device, averaged across the five benchmark datasets. The throughput reduction factor
    (server / edge) is annotated on each edge bar. Higher is better ($\uparrow$).}
    \label{fig:throughput_bar}
\end{figure}

GTA \citep{gta2022} stands out as a clear outlier on the edge device, where its 
average latency of 880.3\,ms and throughput of just 147 windows/s represent a
13.8$\times$ latency increase and 13.7$\times$ throughput reduction respectively, which is by
far the largest degradation of any model evaluated. Even on the server, where it
achieves 64.0\,ms, GTA is the second slowest model despite having a moderate MAC
count, a pattern attributed in Section~\ref{subsubsec:mac_results} to runtime
overhead from its graph construction and adjacency matrix operations. On the edge
device this overhead compounds further.

TimesNet \citep{timesnet2023} degrades the least in relative terms, with a
latency increase factor of only 3.2$\times$ and a throughput reduction of 4.1$\times$.
This is the most graceful degradation of any model, suggesting that its 2D convolutional
operations translate relatively efficiently to the edge hardware. Nevertheless, its
absolute edge latency of 340.4\,ms makes it the second slowest model on the edge device.

Anomaly Transformer \citep{anomalytransformer2022} incurs a 4.1$\times$ latency
slowdown and 8.2$\times$ throughput reduction. Its fixed architecture and consistent
MAC count across datasets appear to result in predictable and moderate scaling
behaviour across hardware environments. (On the server environment, Anomaly 
...
reported factors reflect this deployment setting.)

ModernTCN \citep{moderntcn2024} incurs a 6.0$\times$ latency increase and 4.2$\times$
throughput reduction. While it remains the fastest model in absolute terms on both
platforms, the latency penalty is larger than one might expect for a lightweight
convolutional model, suggesting that some of its efficiency advantage on the server 
is tied to the more powerful 
server GPU that is not available on the edge device.

TranAD \citep{tranad2022} shows a 9.9$\times$ latency increase and 9.2$\times$
throughput reduction, which is the second largest of any model. The result confirms
that the edge device's CPU is substantially less capable than the server CPU,
amplifying the cost of CPU-only execution in a constrained environment.

\section{Evaluation Summary}
Figure~\ref{fig:radar_chart} presents a radar chart summarising the performance 
of all five models across six evaluation dimensions: AU-PR score, F1 score, throughput, 
inference latency, peak inference memory, and peak training memory. All efficiency 
values are taken from the edge device experiments, making this a direct assessment 
of each model's suitability for the target deployment scenario. The values shown 
represent averages across all five benchmark datasets. For metrics where a lower 
value is better --- latency, inference memory, and training memory --- the axes are 
displayed on an inverted scale so that a larger area consistently indicates better 
performance across all dimensions.

\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{radar_chart.png}
    \caption{Radar chart comparing all five models across six evaluation metrics,
    averaged across the five benchmark datasets. For metrics where a lower value
    is preferable --- training memory, inference memory, and latency --- the axes
    use an inverted scale, so that a larger enclosed area consistently indicates
    better overall performance.}
    \label{fig:radar_chart}
\end{figure}

The chart reveals a clear trade-off between detection accuracy and deployment
efficiency. Anomaly Transformer \citep{anomalytransformer2022} achieves the highest
F1 score but its polygon collapses on both memory axes, reflecting its prohibitive
runtime memory demand documented in Section~\ref{subsubsec:runtime_server}. Despite
its accuracy leadership, this makes it the least suitable model for
resource-constrained edge deployment.

TranAD \citep{tranad2022} and ModernTCN \citep{moderntcn2024} emerge as the two
strongest candidates for edge deployment, and the choice between them is not
straightforward. TranAD's polygon extends furthest on the memory axes and on AU-PR,
reflecting its by far the lowest memory footprint of any model and its leading
threshold-independent detection quality. ModernTCN, by contrast, dominates the
throughput axis and achieves the best latency, with a consistently large polygon
across all non-memory dimensions. Both achieve competitive F1 scores.

The choice between the two therefore depends on the specific constraints of the
target deployment. If memory is the primary concern, for instance on devices
with very limited RAM budgets, TranAD is the better choice, as its memory
footprint is in a category of its own. If latency and throughput are the binding
constraints, for example in applications that must process high-rate sensor
streams in real time, ModernTCN is the more suitable option.

GTA \citep{gta2022} and TimesNet \citep{timesnet2023} present the weakest overall
profiles. GTA's polygon collapses on the latency axis, consistent with its
880.3\,ms average edge latency, and it offers no compensating advantage on memory
or accuracy. TimesNet's polygon is similarly constrained, with the smallest
enclosed area of any model driven by its high latency, large memory footprint,
and the lowest F1 of all five models. Neither model offers a compelling case for
edge deployment relative to the alternatives evaluated here.

Taken together, the results support two conclusions. First, no single model
dominates on all metrics simultaneously, confirming that accuracy and efficiency
represent genuinely competing objectives in this benchmark. Second, the choice of
model depends on the deployment context: if memory is the binding constraint,
TranAD is the recommended choice; if latency and throughput take priority,
ModernTCN is the stronger option. Anomaly Transformer, despite its accuracy
leadership, is not a viable candidate for resource-constrained IoT edge deployment
under the conditions evaluated in this thesis.
...
This thesis investigated the suitability of modern deep learning models for
multivariate time-series anomaly detection in resource-constrained IoT
environments. While recent TSAD research has largely focused on maximising
detection accuracy, practical edge deployment introduces additional constraints
that change how models should be evaluated. The central goal of
this work was therefore not only to determine which model performs best on
benchmark datasets, but to understand how architectural design choices influence
computational efficiency, memory behaviour, and real-world deployability under
constrained hardware conditions.

To address this, five representative deep learning models spanning convolutional,
transformer-based, and graph-based architectures were evaluated across two
complementary dimensions: detection performance and computational efficiency.
The selected models — TimesNet \citep{timesnet2023}, ModernTCN \citep{moderntcn2024}, 
TranAD \citep{tranad2022}, Anomaly Transformer \citep{anomalytransformer2022}, and
GTA \citep{gta2022} — represent distinct design choices for learning temporal and
intermetric dependencies in multivariate time series. Their evaluation across
multiple benchmark datasets and hardware environments enabled a comparative
analysis not only of model quality, but also of the trade-offs that emerge
between accuracy, runtime behaviour, and deployment feasibility.

One of the clearest findings throughout the evaluation was that accuracy metrics
alone provide an incomplete and sometimes misleading picture of model quality.
The comparison between point-adjusted F1 and AU-PR demonstrated that models can
perform very differently depending on whether evaluation relies on
threshold-sensitive segment-level credit or threshold-independent score
separability. In particular, the experiments highlighted how point adjustment can
substantially inflate perceived performance, especially for models capable of
detecting only a small portion of a long anomalous segment. Reporting AU-PR
alongside point-adjusted F1 therefore provided a more balanced and informative
evaluation framework, revealing cases where strong F1 performance did not
necessarily correspond to robust anomaly score distributions.

The experiments further showed that architectural design decisions strongly shape
the balance between detection capability and efficiency. The convolution-based
models demonstrated that modern CNN architectures remain competitive for
TSAD despite the dominance of Transformer-based approaches in recent literature.
ModernTCN in particular illustrated how enlarging the effective receptive field
through large-kernel depthwise convolutions can provide strong temporal modelling
while retaining the computational advantages of convolution \citep{moderntcn2024}. Its consistently
strong latency and throughput characteristics across both hardware environments
showed that convolutional architectures can scale efficiently to edge deployment
scenarios without sacrificing competitive detection performance.

TimesNet demonstrated a different convolutional architecture centred around
explicit modelling of periodic structure through FFT-guided 2D transformation \citep{timesnet2023}.
The evaluation showed that modelling intra-period and inter-period variation
separately can produce strong anomaly detection performance. At the same time, 
the experiments showed that a more complex architecture does not automatically 
lead to better deployment efficiency, because the extra tensor reshaping and 
multi-period processing add more computational cost compared to simpler 
convolution-based approaches.

The Transformer-based models revealed a contrasting set of strengths and
weaknesses. TranAD \cite{tranad2022} showed that compact Transformer architectures can achieve an
extremely favourable efficiency profile despite relying entirely on CPU execution
in its original implementation. Its low parameter count, modest memory
requirements, and relatively stable runtime behaviour across datasets demonstrated
that Transformer-based models do not necessarily imply prohibitively expensive
deployment costs. The experiments also highlighted the practical effectiveness of
self-conditioning and adversarial reconstruction amplification for detecting
subtle anomalies without requiring large or computationally intensive
architectures.

Anomaly Transformer demonstrated the benefits of attention-based association
modelling for anomaly detection, introducing a fundamentally different detection
criterion from reconstruction error alone \citep{anomalytransformer2022}. 
The experiments showed that modelling
discrepancies between local and global temporal associations can produce strong
detection capability across diverse datasets. However, the evaluation also
revealed the substantial runtime memory costs associated with Transformer
attention mechanisms, particularly under reduced batch sizes on constrained
hardware. This highlighted an important practical limitation of attention-heavy
architectures for edge deployment: even when parameter counts remain moderate,
runtime activation memory and attention computation can become the dominant
deployment bottleneck.

GTA \citep{gta2022} provided insight into graph-based modelling for IoT anomaly detection. Its
explicit representation of inter-sensor dependencies and learned graph topology
demonstrated the value of incorporating structural information into TSAD
architectures, particularly for systems where anomalies propagate across sensors
rather than appearing independently. At the same time, the evaluation showed that
architectures designed specifically for IoT data are not automatically optimised
for IoT hardware constraints. Despite its IoT-oriented motivation, GTA incurred
substantial runtime overhead due to the combination of graph operations,
hierarchical context encoding, and Transformer-based forecasting. This underlined
a recurring theme throughout the thesis: architectural complexity intended to
improve modelling capacity often introduces deployment costs that may outweigh
the practical benefits in constrained environments.

Beyond individual models, the efficiency evaluation revealed several broader
insights into deployment-oriented TSAD benchmarking. First, parameter count alone
proved insufficient for characterising deployment feasibility. Models with
relatively modest parameter counts could still exhibit very large runtime memory
footprints due to intermediate activations, attention maps, or training graph
overhead. Runtime memory behaviour therefore emerged as a more meaningful
practical constraint than static model size alone.

Second, the comparison between server-grade and edge hardware highlighted that
deployment rankings can change substantially depending on the target environment.
Some models that remained competitive on server hardware degraded
disproportionately on the edge device due to memory pressure
or hardware-specific execution inefficiencies. This reinforced the importance of
evaluating TSAD models directly on representative deployment hardware rather than
extrapolating from server-side benchmarks alone.

Among the five models evaluated, the results of this thesis demonstrate that there is no universally
optimal model for multivariate time-series anomaly detection under IoT
constraints. Instead, deployment suitability depends on which resource constraint
is dominant in the target environment. Models optimised primarily for detection
performance may become impractical when memory budgets or latency requirements
are strict, while highly efficient models may sacrifice detection
quality in more complex anomaly scenarios.

In conclusion, this thesis showed that evaluating anomaly detection models solely
through benchmark accuracy overlooks many of the constraints that determine
real-world usability in IoT systems. By jointly analysing detection performance,
computational cost, memory behaviour, and inference speed across both server and
edge hardware, this work provides a more deployment-oriented understanding of
modern TSAD architectures and the trade-offs that govern their suitability for
practical edge deployment.


\section{Limitations and Future Work}

Although this thesis provides a comprehensive evaluation of modern TSAD models from 
both an accuracy and deployment perspective, several limitations remain that open 
opportunities for future research.

One limitation of this work is the selection of evaluated models. While the 
chosen architectures represent diverse design approaches, the field of time-series 
anomaly detection is evolving rapidly, especially in the area of lightweight and 
edge-oriented models. For example, approaches such as TinyAD \citep{tinyad2023} specifically 
target memory-efficient anomaly detection for Industrial IoT environments. 
However, some of these methods either lacked publicly available implementations or provided insufficient 
reproducibility support at the time of evaluation, making a fair experimental comparison difficult. 
As a result, future work should extend this evaluation framework to include more lightweight 
TSAD architectures.

Another limitation concerns hyperparameter optimisation and reproducibility. Several of 
the evaluated papers did not provide detailed information regarding hyperparameter tuning 
procedures, recommended parameter ranges, or dataset-specific training configurations. 
In some cases, only partial implementation details were available, requiring assumptions 
or independent tuning decisions during reproduction. Consequently, certain models may not have 
achieved their full potential performance on all datasets within this thesis. Future research could 
therefore focus on establishing more standardized and reproducible TSAD benchmarking practices, including 
transparent reporting of tuning strategies, training schedules, and hardware-specific optimisation settings.

The evaluation was also limited to a selected set of benchmark datasets and hardware configurations. 
Although the chosen datasets cover different anomaly detection scenarios, they cannot fully represent 
the diversity of real-world Industrial IoT environments. Future work could expand the analysis to additional 
datasets with varying anomaly characteristics, sensor modalities, and temporal dynamics. Similarly, evaluating 
the models on a broader range of edge hardware platforms, would provide a more complete understanding 
of deployment behaviour under practical constraints.

In addition, this thesis focused primarily on inference-time efficiency metrics such as latency, throughput, 
memory consumption, and computational cost. Other deployment-oriented factors, including power 
consumption and thermal efficiency \citep{fanariotis2023power}, or long-term reliability under 
continuous streaming workloads \citep{li2018energy}, 
were outside the scope 
of this work. These aspects are highly relevant for real-world IoT deployments and represent important 
directions for future research.

Finally, several optimisation techniques were left unexplored due to both time constraints and the need to maintain 
a controlled comparison between the original model architectures. Methods such as quantisation \citep{cheng2023compression}, 
pruning \citep{cheng2023compression} and 
knowledge distillation \citep{hinton2015distilling} could substantially improve 
the deployability of existing TSAD models on edge devices. Future work could investigate how these optimisation 
approaches affect both anomaly detection quality and systems-level efficiency, particularly for Transformer-based 
architectures that showed strong detection capabilities but high runtime overhead.
